...
float sample_volume(float3 p) {
	return tex3Dlod(_VolumeTex, float4(p, 0)).a; // TODO: undeclared identifier '_VolumeTex'
}
...
struct VertexOutputDeferred
{
	...
	half4 ambientOrLightmapUV			: TEXCOORD5;	// SH or Lightmap UVs			
		...
		#endif
	#endif
	// TODO ray marching variables:
	float3 localPos						: TEXCOORD8;
	float3 worldPos						: TEXCOORD9;
	float3 viewRay						: TEXCOORD10;
};
...
VertexOutputDeferred vertDeferred (VertexInput v)
{
	...
	// TODO: delete what might be necessary
...
)
{
	...
	//outNormal = mul(_Object2World, float4(get_normal(current_pos, texelSize),0));
